...
const int NUM_FINDS_PER_TRIAL = 100;
...
void print_stats(const string& label, const vector<uint64_t>& times) {
    ...
    // Print the stats in a readable format (e.g., "Unsorted Vector Find Time Avg: 0.1 us, Min: 0.09 us, Max: 0.12 us")
    cout << "  " << label << " Avg: " << avg_us << " us, Min: " << min_us << " us, Max: " << max_us << " us\n";
}
...
int main() {
    ...
    int trials = 100;  // Number of trials to run for each size to average out noise
